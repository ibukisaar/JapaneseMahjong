...
namespace 日本麻将.Yakus {
	public sealed class 纯全带幺九 : Yaku {
		...
		protected override bool Test(ICollection<YakuValue> result, Tile[] tiles, IGroups groups, YakuEnvironment env) {
			if (groups.Any(g => !g.IsTerminal)) return false;

			result.Add(YakuValue.FromFanValue("纯全带幺九", (env & YakuEnvironment.门前清) != 0 ? 3 : 2));
			return true;
		}
	}
}
